...
On a fresh dev box, `scripts/setup.sh` is the one-command bootstrap —
installs Firecracker, builds the guest agent, fetches/builds an image,
injects the agent, creates the tap pool, grants `CAP_NET_ADMIN`, all
idempotent. After that, `scripts/sandkilnd-ctl.sh start` needs no env
vars at all (it sources `.env.sandkiln-setup`, which `setup.sh` writes)
and is the one-command way to get a real daemon up for manual poking
(builds, preflight-checks, starts, waits for `/healthz`) —
`restart`/`stop`/`status`/`logs` round it out. See `SELF_HOSTING.md`'s
"Quick start" and section 8; these are the same tools a self-hoster uses,
not a separate dev-only path.
