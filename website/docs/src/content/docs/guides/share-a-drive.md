...
Repeat this against as many sandboxes as you want — every one succeeds, concurrently, as long as every existing holder and every new attach are all `read_only: true`. A single read-write attach — existing or newly requested — still needs exclusive access; see [Drives](/docs/concepts/drives/) for the exact rule.
