...
A jailed sandbox, or one forked from a snapshot (shares its rootfs with the snapshot it came from) — both are structurally ineligible for the same reasons they can't be snapshotted manually either. See [Snapshots, resume, and fork](/docs/concepts/snapshots/).
