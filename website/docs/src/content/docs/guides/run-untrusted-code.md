...
- **For genuinely adversarial workloads, turn on the jailer.** `SANDKILN_JAILER_ENABLED` adds chroot, cgroup v2 resource limits, and a dedicated unprivileged uid per VM — see [Privilege model](../architecture/privilege-model/). It's opt-in and not yet proven against a real installed jailer binary on hardware, so verify it in your own environment before relying on it for something genuinely hostile.
