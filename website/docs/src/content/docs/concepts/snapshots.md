...
- `DELETE /sandboxes/:id` (the default, `keep=true`) does the same thing internally, triggered by a stop rather than an explicit snapshot call. See [Sandbox lifecycle](/docs/concepts/sandbox-lifecycle/).
- Auto-suspend does it automatically for an idle sandbox, if configured. See [Auto-suspend idle sandboxes](/docs/guides/auto-suspend/).
