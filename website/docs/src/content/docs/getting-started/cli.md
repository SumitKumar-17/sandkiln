...
`--base-url`/`--token` default to the `SANDKILN_DAEMON_URL`/`SANDKILN_AUTH_TOKEN` environment variables when omitted, same as both SDKs — see [Auth](/docs/concepts/auth/). Full command reference: [CLI (kiln)](/docs/reference/cli/).
