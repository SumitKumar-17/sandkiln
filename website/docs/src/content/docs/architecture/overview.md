...
Measured end to end: 32.3–33.1ms. See [Startup latency & the pre-warmed pool](/docs/architecture/startup-latency/) for what's next after boot itself is already this fast.

## Deeper dives

Each of the following came from hitting a real constraint on real hardware, not a whiteboard preference:

- [Privilege model](/docs/architecture/privilege-model/) — ambient `CAP_NET_ADMIN`, a static tap pool, and a root that stays out of the hot path.
- [Wire protocol](/docs/architecture/wire-protocol/) — vsock, a length-prefixed JSON frame, and a client that doesn't reach for a full HTTP stack.
- [Persistence model](/docs/architecture/persistence-model/) — sandbox vs. session, and why a snapshot points at paths instead of carrying values.
- [Bug hunt: the vsock timeout](/docs/architecture/bug-hunt-vsock-timeout/) — case study of a real bug that could hang a stop forever.
