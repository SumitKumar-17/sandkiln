...
- **For genuinely adversarial workloads, turn on the jailer.** `SANDKILN_JAILER_ENABLED` adds chroot, cgroup v2 resource limits, and a dedicated unprivileged uid per VM — see [The jailer and privilege model](../../internals/jailer-privilege-model/). It's opt-in and not yet proven against a real installed jailer binary on hardware, so verify it in your own environment before relying on it for something genuinely hostile.
- **Set an idle timeout.** `SANDKILN_IDLE_TIMEOUT_SECS` or `SANDKILN_AUTO_SUSPEND_TIMEOUT_SECS` on the daemon reclaims a sandbox a caller forgot to stop — see [Auto-suspend idle sandboxes](../auto-suspend/).
