...
A sandbox that structurally can't be preserved (booted jailed — see [The jailer and privilege model](../../internals/jailer-privilege-model/)) falls back to a full destroy automatically rather than erroring or leaking resources. A sandbox forked from a snapshot (see [Snapshots, resume, and fork](../snapshots/)) has nothing new to preserve either — its state already lives in the snapshot it came from — so it's silently destroyed on the default path too, reported as `{"kept": false}`.
